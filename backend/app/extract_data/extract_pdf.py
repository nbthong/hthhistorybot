...
def load_local_db() -> list:
    ...
    if not os.path.exists(OUTPUT_JSON_NEW) or os.path.getsize(OUTPUT_JSON_NEW) == 0:
        return []

    try:
        with open(OUTPUT_JSON_NEW, "r", encoding="utf-8") as f:
            return json.load(f)
    except json.JSONDecodeError as e:
        backup_path = f"{OUTPUT_JSON_NEW}.bak"
        try:
            os.replace(OUTPUT_JSON_NEW, backup_path)
...
def save_local_db(local_db: list) -> None:
    ...
    with open(OUTPUT_JSON_NEW, "w", encoding="utf-8") as f:
