...
def embed_and_store_lessons(
    merged: Dict[str, Any],
    *,
    mongo_collection_name: str = MONGO_VECTOR_COLLECTION_GEMINI,
) -> None:
...
if __name__ == "__main__":
    ...
    merge_file = OUTPUT_JSON_CHUNK_EMBEDDING_GEMINI
    if not merge_file.exists():
        logger.error(f"File {merge_file} does not exist: {merge_file}")
        logger.info(f"Run preprocessing.py before to create {merge_file}")
        exit(1)
    ...
    embed_and_store_lessons(merged, mongo_collection_name=MONGO_VECTOR_COLLECTION_GEMINI)